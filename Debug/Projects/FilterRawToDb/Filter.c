/* ------------------------------------
Title: RawToDb filter

Author: Franck Charlet

Purpose: Convert raw binary datas into assembly db statements
...
LPFILTERFILE __stdcall FilterProc(char *FileName, LPFILTERFILE File, LPCHROMATICLIB CL)
{
    ...
    /* Convert main part */
    if(File_Len > 15) CL->StringFillHexBytes(File->FileMem, Output_Buffer, 16, 5, 5, File_Len16, 3);
    /* Convert remaining bytes */
    if(File_Remainder != 0) CL->StringFillHexBytes(((char *) File->FileMem + (File_Len - File_Remainder)), Output_Buffer_End, File_Remainder, 5, 0, 1, 3);
    /* Write the header */
