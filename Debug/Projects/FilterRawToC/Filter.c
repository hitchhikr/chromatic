...
LPFILTERFILE __stdcall FilterProc(char *FileName, LPFILTERFILE File, LPCHROMATICLIB CL)
{
    ...
    /* Convert main part */
    if(File_Len > 15) CL->StringFillHexBytes(File->FileMem, Output_Buffer, 16, 3, 3, File_Len16, 3);
    /* Convert remaining bytes */
    if(File_Remainder != 0) CL->StringFillHexBytes(((char *) File->FileMem + (File_Len - File_Remainder)), Output_Buffer_End, File_Remainder, 3, 0, 1, 3);
    /* Write the header */
    Output_Buffer_Final = CL->StringCat("/*\r\nCreated from file: ", FileName);
    ...
    Output_Buffer_Final = CL->StringCat(Output_Buffer_Final, "#define ");
    Output_Buffer_Final = CL->StringCat(Output_Buffer_Final, FileName_Label);
    Output_Buffer_Final = CL->StringCat(Output_Buffer_Final, "_size ");
    Output_Buffer_Final = CL->StringCat(Output_Buffer_Final, CL->StringDecToString(File_Len));
    Output_Buffer_Final = CL->StringCat(Output_Buffer_Final, "\r\n");
    Output_Buffer_Final = CL->StringCat(Output_Buffer_Final, "unsigned char ");
    Output_Buffer_Final = CL->StringCat(Output_Buffer_Final, FileName_Label);
    ...
    if(File_Remainder == 0) Output_Buffer_Final = CL->StringCat(Output_Buffer_Final, "};\r\n");
    else Output_Buffer_Final = CL->StringCat(Output_Buffer_Final, "\r\n};\r\n");
    /* Set new buffer */
